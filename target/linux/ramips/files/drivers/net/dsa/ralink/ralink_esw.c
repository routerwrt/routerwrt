...
static void ralink_esw_vlan_write(struct ralink_esw *esw, int idx)
{
	...
	ralink_esw_set_vlan_untag(esw, idx, esw->vlan[idx].untag);
}

static int ralink_esw_find_vlan_idx(struct ralink_esw *esw, u16 vid)
{
	int i;

	for_each_set_bit(i, esw->vlan_idx, RALINK_ESW_NUM_VLANS) {
		if (esw->vlan[i].vid == vid)
			return i;
	}
...
static int ralink_esw_alloc_vlan_idx(struct ralink_esw *esw, u16 vid)
{
	int i;

	i = find_first_zero_bit(esw->vlan_idx, RALINK_ESW_NUM_VLANS);
	if (i >= RALINK_ESW_NUM_VLANS)
		return -ENOSPC;

	set_bit(i, esw->vlan_idx);

	esw->vlan[i].vid = vid;
	esw->vlan[i].member = 0;
	esw->vlan[i].untag = 0;
	esw->vlan[i].bridge_num = -1;
...
static void ralink_esw_free_vlan_idx(struct ralink_esw *esw, int i)
{
	clear_bit(i, esw->vlan_idx);

	esw->vlan[i].vid = RALINK_ESW_VID_NONE;
	esw->vlan[i].member = 0;
	esw->vlan[i].untag = 0;
	esw->vlan[i].bridge_num = -1;
...
static inline u16 ralink_esw_vlan_from_idx(struct ralink_esw *esw, int idx)
{
	...
	if (!test_bit(idx, esw->vlan_idx))
		return RALINK_ESW_VID_NONE;

	return esw->vlan[idx].vid;
}

static int ralink_esw_vlan_to_idx(struct ralink_esw *esw, u16 vid)
{
	return ralink_esw_find_vlan_idx(esw, vid);
}
					  ...
					  struct netlink_ext_ack *extack)
{
	struct ralink_esw *esw = ds->priv;
	u32 mask, set;
	bool untag = !vlan_filtering;
	u32 mode = esw->ports[port].port_mode;

	pr_info("port[%d] filtering %d port mode: %d\n",
		port, vlan_filtering, mode);

	if (mode == ESW_VLAN_ACCESS) {
		if (vlan_filtering && esw->ports[port].num_vlans > 1) {
			pr_info("port[%d] is access num vlan: %d\n", 
				port, esw->ports[port].num_vlans);
			return -EBUSY;
		}
	}
	...
	if (untag) {
		mask = RALINK_ESW_POC2_UNTAG_EN_BIT(port);
		set = untag ? mask : 0;
		ralink_esw_rmw(esw, RALINK_ESW_POC2, mask, set);
	}

	pr_info("untag[%d] mask %02x, set %02x\n", port, mask, set);

	esw->ports[port].vlan_filtering = vlan_filtering;
	esw->ports[port].untag = untag;
				    ...
				    struct netlink_ext_ack *extack)
{
	...
	int mode = esw->ports[port].port_mode;
	int port_vid= esw->ports[port].pvid_vlan_filtering;
	u16 vid = vlan->vid;
	u32 mask;
	int idx;
	...
	if (mode == ESW_VLAN_TRUNK && pvid) {
		pr_info("port % is Trunk and wants PVID\n", port);
		return -EBUSY;
	}

	if (mode == ESW_VLAN_ACCESS && !pvid && vid != port_vid) {
		pr_info("port % is access and VID %d  != PVID %d num %d \n",
			port, vid, port_vid, esw->ports[port].num_vlans);
		return -EBUSY;
	}
	...
	pr_info("port[%d] num vlans committed:  %d\n", port,
				esw->ports[port].num_vlans);
	...
	if (pvid) {
		...
		if (untagged) {
			mask = RALINK_ESW_POC2_UNTAG_EN_BIT(port);
			ralink_esw_rmw(esw, RALINK_ESW_POC2, mask, mask);

			pr_info("untag[%d] mask %02x, set %02x\n", port, mask, mask);

			esw->ports[port].port_mode = ESW_VLAN_ACCESS;
			esw->ports[port].untag = true;
		}

		return ralink_esw_port_commit_pvid(esw, port);
	} else 	if (!untagged && esw->ports[port].vlan_filtering) {
		mask = RALINK_ESW_POC2_UNTAG_EN_BIT(port);
		ralink_esw_rmw(esw, RALINK_ESW_POC2, mask, 0);
		esw->ports[port].untag = false;
		pr_info("untag[%d] mask %02x, set %02x\n", port, mask, 0);
	}
				    ...
				    const struct switchdev_obj_port_vlan *vlan)
{
	...
	if (vid == esw->ports[port].pvid_vlan_filtering) {
		...
		if (esw->soc->port_untag)
			esw->ports[port].port_mode = ESW_VLAN_TRUNK;
	}
...
static void ralink_esw_port_set_default_queue(struct ralink_esw *esw,
						int port, u8 queue)
{
	u32 mask = RALINK_ESW_PFC1_PORT_PRI_MASK(port);
	u32 set = RALINK_ESW_PFC1_PORT_PRI_VAL(port, queue & 0x3);

	if (!esw->soc->port_untag)
		ralink_esw_rmw(esw, RALINK_ESW_PFC1, mask, set);

	mask = RALINK_ESW_SGC2_LAN_PMAP_BIT(port);
	set = queue ? mask : 0;	
	ralink_esw_rmw(esw, RALINK_ESW_SGC2, mask, set);
}
					...
					struct netlink_ext_ack *extack)
{
	...
	ralink_esw_port_set_default_queue(esw, port, 1);
					...
					struct dsa_bridge bridge)
{
	...
	ralink_esw_port_set_default_queue(esw, port, 0);
}
...
static int ralink_esw_setup(struct dsa_switch *ds)
{
	 ...
	 *
	 * Per-port unknown unicast/multicast host flooding may be disnabled
...
//		RALINK_ESW_SGC2_CPU_TPID_EN |
	...
	*/
	...
	ralink_esw_rmw(esw, RALINK_ESW_POC2,
		...
		FIELD_PREP(RALINK_ESW_POC2_UNTAG_EN, esw->user_port_mask) |
	...
	for (i = 0; i < ds->num_ports; i++) {
		...
		if (esw->soc->port_untag)
			esw->ports[i].port_mode = ESW_VLAN_ACCESS;
		else
			esw->ports[i].port_mode = ESW_VLAN_HYBRID;
		...
		ralink_esw_port_set_default_queue(esw, i, 0);
	}
...
static int ralink_esw_irq_init(struct ralink_esw *esw)
{
	...
	ralink_esw_w32(esw, RALINK_ESW_IMR, 0xffffffff);
	ralink_esw_w32(esw, RALINK_ESW_ISR, 0xffffffff);
